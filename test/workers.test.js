...
const makeQueue = () => {
  ...
  return {
    ...
    process: (name, concurrency, fn) => { order.push(`process:${name}`); processors.set(name, { concurrency, fn }); },
    on:      (event, fn) => { order.push(`on:${event}`); handlers.set(event, fn); },
    close:   async function () { this.closed = true; },
    getJobCounts: async function () { return this.counts; },
...
const { startWorkers, stopWorkers, workerStatus } = require(path.join(ROOT, 'workers/index.js'));
...
test('bull workers', async (t) => {
  ...
  // This ordering is why the first version of this file took the whole API
  // down: an 'error' event with no listener yet is rethrown, and server.js
  // turns an uncaught exception into process.exit(1).
  await t.test("the error listener is attached BEFORE the queue starts processing", () => {
    for (const q of [notificationQueue, auditQueue]) {
      assert.equal(q.order[0], 'on:error', `first call on the queue was ${q.order[0]}`);
      assert.ok(q.order.indexOf('on:error') < q.order.findIndex(o => o.startsWith('process:')));
    }
  });
  ...
  await t.test('a queue error is recorded rather than thrown, and surfaces on the health check', async () => {
    notificationQueue.handlers.get('error')(new Error('ECONNRESET talking to Redis'));
    const status = await workerStatus();
    assert.equal(status.notifications.status, 'degraded');
    assert.match(status.notifications.lastError, /ECONNRESET/);
  });

  await t.test('the health check reports queue depth so a stalled consumer is visible', async () => {
    auditQueue.counts = { waiting: 7, active: 1, completed: 3, failed: 2, delayed: 0 };
    const status = await workerStatus();
    assert.equal(status.running, true);
    assert.equal(status['audit-logs'].waiting, 7);
    assert.equal(status['audit-logs'].failed, 2);
  });
